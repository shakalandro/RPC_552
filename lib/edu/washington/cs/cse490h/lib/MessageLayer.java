...
public class MessageLayer {
    ...
    /**
     * Failure rate for RPCNode (0 - 100)
     */
    @Option(value = "-w Failure rate")
    public static int rpcFail = 0;

    /**
     * Recover rate for RPCNode (0 - 100)
     */
    @Option(value = "-x Recover rate")
    public static int rpcRecover = 0;

    /**
     * Drop rate for RPCNode (0 - 100)
     */
    @Option(value = "-y Drop rate")
    public static int rpcDrop = 0;

    /**
     * Delay rate for RPCNode (0 - 100)
     */
     ...
     */
    @Option(value = "-f Failure level, a number between 0 and 4",
            aliases = { "-failure-lvl" })
    ...
    @Option(value = "-L Synoptic totally ordered log filename",
            aliases = { "-synoptic-totally-ordered-logfile" })
    // TODO: specify a sane default
     ...
     */
    @Option(value = "-l Synoptic partially ordered log filename",
            aliases = { "-synoptic-partially-ordered-logfile" })
    // TODO: specify a sane default
    ...
    private static String usage_string = "java MessageLayer [options]";
    ...
    public static void main(String[] args) {
        ...
        if (synopticTotalOrderLogFilename.equals("")) {
            // printWarning("you did not specify a totally ordered synoptic log file.");
            // // TODO: re-enable when it's working
        } else {
            System.out.println("synopticTotalLogFilename = "
                    + synopticTotalOrderLogFilename);
        }

        if (synopticPartialOrderLogFilename.equals("")) {
            // printWarning("you did not specify a partially ordered synoptic log file.");
            // // TODO: re-enable when it's working
        } else {
            System.out.println("synopticPartialLogFilename = "
                    + synopticPartialOrderLogFilename);
        }
        ...
        if (failureLvlInt == -1) {
            ...
            FailureLvl[] possibleFailureLvls = { FailureLvl.NOTHING, // 0
                    FailureLvl.CRASH, // 1
                    FailureLvl.DROP, // 2
                    FailureLvl.DELAY, // 3
        ...
        try {
            ...
            Class<? extends Node> nodeImpl = ClassLoader.getSystemClassLoader()
                    .loadClass(nodeClass).asSubclass(Node.class);

            if (simulate) {
                if (commandFile.equals("")) {
                    // Simulation replay only really needs to record user input
                    if (replayOutputFilename.equals("")
                            && replayInputFilename.equals("")) {
                        ...
                        return;
                    } else if (!replayOutputFilename.equals("")
                            && !replayInputFilename.equals("")) {
                ...
                try {
                    ...
                    printError("Illegal arguments given to Simulator. Exception: "
                            + e);
                    return;
                } catch (FileNotFoundException e) {
                    printError("Incorrect command file name given to Simulator. Exception: "
                            + e);
                    return;
                }
                ...
                try {
                    if (!commandFile.equals("")) {
                        manager = new Emulator(nodeImpl, nodeAddr,
                                routerHostname, routerPort, failureLvl, seed,
                                timestep, replayOutputFilename,
                                replayInputFilename, commandFile);
                    } else {
                        manager = new Emulator(nodeImpl, nodeAddr,
                                routerHostname, routerPort, failureLvl, seed,
                                timestep, replayOutputFilename,
                    ...
                    printError("Encountered exception while trying to connect to Router. Exception "
                            + e);
                    return;
                } catch (IllegalArgumentException e) {
                    printError("Illegal arguments given to Emulator. Exception: "
                            + e);
                    return;
                }
            }
