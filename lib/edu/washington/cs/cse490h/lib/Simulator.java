...
/**
 * Manages a simulation, where all nodes are running in the same process, in the same thread
 */
public class Simulator extends Manager {
	...
	/**
	 * Base constructor for the Simulator. Does most of the work, but the command input method and
	 * failure level should be set before calling this constructor.
	 * 
	 * @param nodeImpl The Class object for the student's node implementation
	 * @param seed Seed for the random number generator. Can be null to use the current time as a
	 *            seed
	 * @param replayOutputFilename The log file for future relays of the current execution
	 * @param replayInputFilename The log file to replay
	 * @throws IllegalArgumentException If the arguments provided to the program are invalid
	 ...
	 * @param seed Seed for the RNG. This can be null.
	 * @param replayOutputFilename The log file for future relays of the current execution
	 * @param replayInputFilename The log file to replay
	...
	/**
	 * Start up a node, crashed or brand new. If the node is alive, this method will crash it first.
	 * 
	 * @param node The address at which to start the node
	 */
	...
	/**
	 * Fail a node. This method updates data structures, removes the failed nodes's timeouts and
	 * calls its fail() method
	 * 
	 * @param node The node address to fail
	 * @return The exception thrown after calling the fail() method. This is so, if the stack
	 *         includes methods in Node, we can rethrow the Exception as necessary
	 */
	...
	/**
	 * Check to see if any timeouts are supposed to fire during the current time step
	 * 
	 * @param currentRoundEvents The list of the current round's events that we should add to
	 */
	...
	/**
	 * Reorders and executes all the events for the current round. Note that commands can be
	 * executed in a different order than they appear in the command file!
	 * 
	 * @param currentRoundEvents The list of the current round's events that we should add to
	 */
	 ...
	 * Process an event.
	 * 
	 * @param ev The event that should be processed
	 */
	private void handleEvent(Event ev) {
		...
		case TIMEOUT:
			...
			if (!cleanOutput) {
				logEventWithNodeField(ev.to.node, "TIMEOUT fire-time:" + ev.to.fireTime + " "
					+ ev.to.cb.toSynopticString());
			}
	...
	/**
	 * Create a packet and put it on the channel. Crashes in the middle of a broadcast can be
	 * modeled by a post-send crash, plus a sequence of dropped messages
	 * 
	 * @param fromNode The node that is sending the packet
	 * @param to Integer specifying the destination node
	 * @param protocol The protocol of the message
	 * @param payload The payload to be sent, serialized to a byte array
	 * @throws IllegalArgumentException If the send is invalid
	 */
	@Override
	 ...
	 * 
	 * @param destAddr The address of the recipient
	 * @param destNode The node object of the recipient
	 * @param srcAddr The address of the sender
	 * @param pkt The packet that should be delivered
	 */
	...
	/**
	 * Check whether a given node is live and therefore valid to give msgs/cmds. Additionally, an
	 * error message will print out if the address itself is invalid.
	 * 
	 * @param nodeAddr The node for which we want to check validity
	...
	/**
	 * Log the event in the synoptic log using the simulator's global logical ordering with a node
	 * field.
	 * 
	...
	/**
	 * Log the event in the synoptic log using the simulator's global logical ordering, and call
	 * super.logEvent to log the event using the implicit partial ordering.
	 * 
	 * @param node node generating the event
	 * @param eventStr the event string description of the event
	 */
	...
	/**
	 * Logs a simulator event across ALL the simulated nodes. The TIMESTEP event is of this form --
	 * its reported for every node that is simulated.
	 * 
	 * @param eventStr the event string description of the event
	 */
